...
from src.seller.database import get_seller_db, SellerAuditLog
...
from src.utils.delta_sharing_utils import (
    extract_token_from_header, validate_share_access,
    get_table_path_from_share, transform_schema_for_timestamp_ntz,
    create_delivery_token, build_delivery_url, get_delivery_base_url,
    cleanup_old_watermarked_files, get_share_from_token,
    validate_schema_and_table, get_expected_table_name, EXPECTED_SCHEMA_NAME,
)
...
DEFAULT_SYNTHETIC_DP_EPSILON = None
...
async def list_shares(
    ...
    return {
        "items": [{"name": f"share_{share.share_id}"}],
        "nextPageToken": None
    }

@router.get("/shares/{share_name}/schemas")
...
@router.get("/shares/{share_name}/schemas/{schema_name}/tables")
async def list_tables(
    ...
    table_name = get_expected_table_name(share)
...
@router.get("/shares/{share_name}/schemas/{schema_name}/tables/{table_name}/metadata")
async def get_table_metadata(
    ...
    share = get_share_from_token(token, db)
    validate_share_access(share_name, share)
    validate_schema_and_table(share, schema_name, table_name)

    bucket_name = get_bucket_name()
    resolved_table_path = get_table_path_from_share(share, bucket_name)

    try:
        storage_options = get_delta_storage_options()
        delta_table = DeltaTable(resolved_table_path, storage_options=storage_options)
        ...
        logger.error(f"[delta-metadata] unexpected error: {type(e).__name__}: {e}")
        raise HTTPException(status_code=500, detail="Error reading Delta table")

@router.get("/shares/{share_name}/schemas/{schema_name}/tables/{table_name}/version")
async def get_table_version(
    ...
    try:
        ...
        logger.error(f"[delta-version] unexpected error: {type(e).__name__}: {e}")
        raise HTTPException(status_code=500, detail="Error reading Delta table version")

@router.post("/shares/{share_name}/schemas/{schema_name}/tables/{table_name}/query")
async def query_table(
    ...
    table_path = get_table_path_from_share(share, bucket_name)
    table_prefix = share.table_path.rstrip('/')
    ...
    nonce = share.watermark_nonce or ""
    watermark = generate_watermark(share.seller_id, share.dataset_id, nonce=nonce)
    ...
    try:
        ...
        if share.is_trial:
            synthetic_path = f"{table_prefix}/_synthetic_trial_{share.share_id}"
            ...
            try:
                ...
                synthetic_df, _ = generate_synthetic_data(
                    original_table_path=share.table_path,
            ...
            watermarked_prefix = f"{table_prefix}/_watermarked_{share.share_id}_"
        ...
        anchor_columns_list = parse_anchor_columns(share.anchor_columns)
        ...
        server_max = get_settings().SELLER_MAX_QUERY_ROWS
        buyer_limit = read_limit if read_limit is not None else effective_limit
        if buyer_limit is None:
            limit_to_use = server_max
            limit_is_server_cap = True
        else:
            limit_to_use = min(buyer_limit, server_max)
            limit_is_server_cap = limit_to_use == server_max and server_max < buyer_limit

        for batch in scanner.to_batches():
            if rows_scanned >= limit_to_use:
                break
        ...
        truncated_by_server_cap = limit_is_server_cap and rows_scanned >= server_max
        ...
        if len(available_anchor_cols) != len(anchor_columns_list):
            ...
            logger.warning(f"Warning: Anchor columns missing from query result: {missing}. Available: {available_anchor_cols}")
        ...
        temp_key = f"{table_prefix}/_watermarked_{share.share_id}_{uuid.uuid4().hex[:8]}.parquet"
        ...
        try:
            ...
            logger.warning(f"Warning: Could not get metadata from DeltaTable: {e}")
            metadata = None
            table_version = 0
        ...
        for file_path in files_list:
            ...
            delivery_token = create_delivery_token(share_id=share.share_id, key=key, kind="query_file")
            delivery_url = build_delivery_url(get_delivery_base_url(request), delivery_token)
        ...
        try:
            ...
            seal_audit_entry(db, audit_log)
            db.add(audit_log)
            db.commit()
        except Exception as e:
            logger.warning(f"Warning: Failed to log query: {e}")
        response_headers = {"delta-table-version": str(table_version)}
        if truncated_by_server_cap:
            response_headers["X-Row-Cap-Applied"] = str(server_max)
        return StreamingResponse(
            io.BytesIO(content.encode()),
            media_type="application/x-ndjson",
        ...
        logger.error(f"[delta-query] unexpected error: {type(e).__name__}: {e}")
        logger.info(traceback.format_exc())
        raise HTTPException(
            status_code=500,
            detail="Internal server error while processing Delta Sharing query",
        )
